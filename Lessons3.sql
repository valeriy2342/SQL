...
ALTER TABLE `profiles` ADD CONSTRAINT fk_photo_id
 ...
 -- Добавляю таблицы для организации видеовльбомов и видеофайлов
...
CREATE TABLE users_games(
	...
	PRIMARY KEY (user_id, game_id), -- чтобы не было 2 записей о пользователе и сообществе
    FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (game_id) REFERENCES games(id) ON UPDATE CASCADE ON DELETE CASCADE
);

-- Добавляю таблицы запросов групп
